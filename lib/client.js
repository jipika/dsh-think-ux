...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const MAIN_SMOOTH_FOLLOW = false;
		...
		function apply(ctx) {
			...
			/*
			 * The bundle's disclosure body is a grid 0fr->1fr height animation
			 * (.disclosureContent{display:grid;grid-template-rows:0fr|1fr} plus
			 * `> *{min-height:0;overflow:hidden}`). A max-height on the grid
			 * CONTAINER collapses that track: once content exceeds the cap the
			 * 1fr row resolves toward 0 (min-height:0 wins intrinsic sizing),
			 * the box shrinks to its own padding and nothing is scrollable —
			 * the "think box went nearly blank / can't scroll" bug. So the
			 * cap and the scroll live on the grid CHILD (the real content
			 * box), which the 0fr/1fr animation then sizes to
			 * min(content, cap): collapsed state, the height transition and
			 * the fixed 24-line preview all stay intact.
			 */
			const THINK_INNER_SEL = THINK_BODY_SEL + " > *";
			...
			function bodyOf(row) {
				...
				if (d === null || d.nextElementSibling === null) return null;
				const box = d.nextElementSibling;
				/* The animated disclosure wraps the content in one grid child;
				 * that child carries the cap and is the scroller (see the
				 * capCss note). Bare-body structures fall back to the box. */
				return box.firstElementChild !== null ? box.firstElementChild : box;
			}
